...
terminal setup, and wanted to experiment with applying the theme to my selfhosted
...
Miniflux offers primitive CSS overrides via the settings page, so the goals for this experiment
were:
...
When crafting the final prompt, I needed to:

- Split it in two as the CSS code block hit the character limit for the `v3.5-turbo` model
- Instruct GPT to not include any explanation do what it was doing, as it was explaining why each
  colour was used
...
Amend the `#blank` CSS values to create a new theme:
...
```
Apply the same change to these CSS values (`#blank`) following the same theme as before:
...
without any existing colour values. I found the limitations of the `v3.5-turbo` model the most
interesting part:
